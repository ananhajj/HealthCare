...
export function BookingCard({ booking }) {
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setCanStartCall(now >= bookingTime && booking.status === 'confirmed');
    ...
    <div className="w-full p-6 border rounded-xl shadow-lg bg-white hover:shadow-2xl transition-shadow duration-300 group">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="relative flex-shrink-0">
          ...
          <span
            className={`absolute -top-3 -right-3 px-3 py-1 rounded-full text-sm flex items-center gap-2 shadow-md ${getStatusColor(booking.status)}`}
          >
        ...
        <div className="flex-1 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 text-gray-600">
              ...
              <span>
                {new Date(booking.bookingTime).toLocaleString('ar-SA', {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
